  ...
  return [
    {
      ...
      description:
        "In this lesson, we'll learning how to solve the <b><i>Second Layer</i></b>. This will involving positioning 4 edge pieces in the middle layer. Each edge piece should be positioned correctly between its matching center colors.\n\nTake the time to look at the cube to understand what we're aiming to achieve here then click <b><i>Next</i></b> to move on to the next slide.",
    ...
    },
    {
      ...
      description:
        "In this lesson, we'll be focusing on the <b><i>Red/Green</i></b> second layer edge piece. Notice how it needs to be positioned between the <b><i>Red</i></b> and <b><i>Green</i></b> center pieces.\n\n<b>The algorithms we'll be using are as follows:</b>\n• <b><i>'Insert right'</i></b>: U / R U R' U' / L' U' L U\n• <b><i>'Insert left'</i></b>: U' / L' U' L U / R U R' U'.\n\nIn these lessons, well be using a combination of <b><i>'Righty'</i></b> and <b><i>'Lefty'</i></b> algorithms depending on the situation to solve the second layer. Click <b><i>Next</i></b> to move on to the next slide.",
      allowFaceMoves: true,
      setup: (cube: CubeJSWrapper) => {
    ...
    },
    {
      ...
      description:
        "So here, what we need to do is look around the top layer for an edge piece <b><i>'without yellow'</i></b> on it. Here, we have <b><i>Red/Green</i></b>. After we've found one, we match the color which is on the side with the center color (which is what we have here with <b><i>Green</i></b> matching the <b><i>Green</i></b> center).\n\nLooking at the <b><i>Red/Green</i></b> edge piece here, we need to determine which slot we need to insert it into. In this case, we need to insert it into the <b><i>Left</i></b> slot to it between with the <b><i>Green</i></b> & <b><i>Red</i></b> centers. To do this, we'll use the <b><i>'Insert left'</i></b> algorithm.\n\n<b>This algorithm is performed by doing:</b>\n• <b><i>U'</i></b> - Move in the opposite direction of the slot we're inserting into.\n• <b><i>L' U' L U</i></b> - The 'Lefty' algorithm with green facing front.\n• <b><i>R U R' U'</i></b> - The 'Right' algorithm with red facing front.\n\nLet's try this to see if we can insert the edge piece correctly, practice this several times until you're comfortable with it and really understand the mental model. After you're happy with it, click <b><i>Next</i></b> to move on to the next slide.",
      allowFaceMoves: true,
      setup: (cube: CubeJSWrapper) => {
        cube.reset();
    ...
    },
    {
      ...
      description:
        "This is the same as the previous slide, but with the <b><i>Right</i></b> slot. First, we match the color which is on the side with the center color (which is what we have here with <b><i>Red</i></b> matching the <b><i>Red</i></b> center).\n\nWe then need to determine which way we need to insert the <b><i>Red/Green</i></b> edge piece. In this case, we need to insert it into the <b><i>Right</i></b> slot to it between with the <b><i>Green</i></b> & <b><i>Red</i></b> centers. To do this, we'll use the <b><i>'Insert right'</i></b> algorithm.\n\n<b>This algorithm is performed by doing:</b>\n• <b><i>U</i></b> - Move in the opposite direction of the slot we're inserting into.\n• <b><i>R U R' U'</i></b> - The 'Righty' algorithm with red facing front.\n• <b><i>L' U' L U</i></b> - The 'Lefty' algorithm with green facing front.\n\nThese are the same solution as the previous slide, but in the opposite direction. Let's try this to see if we can insert the edge piece correctly, practice this several times until you're comfortable with it and really understand the mental model. After you're happy with it, click <b><i>Next</i></b> to move on to the next slide.",
      allowFaceMoves: true,
      setup: (cube: CubeJSWrapper) => {
        cube.reset();
    ...
    },
    {
      ...
      description:
        "Here we have the <b><i>Orange/Green</i></b> edge piece that we need to insert between the <b><i>Green</i></b> and <b><i>Orange</i></b> centers using a right insert. However, notice that the <b><i>Red/Green</i></b> edge piece is already in that spot.\n\nThis is actually perfect! When we insert the <b><i>Orange/Green</i></b> edge piece using the algorithms we've learned, it will automatically remove the <b><i>Red/Green</i></b> edge piece from its position.\n\nOnce the <b><i>Red/Green</i></b> edge piece is out, we then do a <b><i>U2</i></b> to move it so that the <b><i>Green</i></b> sticker on the side of the edge piece matches the <b><i>Green</i></b> center. Then we perform a right insert to insert the <b><i>Red/Green</i></b> edge piece into its correct position.\n\n<b><i>We'll do this in multiple steps:</i></b>\n• <b><i>U</i></b> - Move the <b><i>Orange/Green</i></b> edge piece in the opposite direction of the slot we're inserting into.\n• <b><i>'Righty Algorithm' (Green front)</i></b> then <b><i>'Lefty Algorithm' (Orange front)</i></b> to insert the <b><i>Orange/Green</i></b> edge piece into the right slot, which removes the <b><i>Red/Green</i></b> edge piece.\n• <b><i>U2</i></b> - Move the <b><i>Red/Green</i></b> edge piece so the <b><i>Red</i></b> sticker on the side matches the <b><i>Red</i></b> center.\n• <b><i>U</i></b> - Move the <b><i>Red/Green</i></b> edge piece in the opposite direction of the slot we're inserting into.\n• <b><i>'Righty Algorithm' (Red front)</i></b> then <b><i>'Lefty Algorithm' (Green front)</i></b> to insert the <b><i>Red/Green</i></b> edge piece into the right slot.\n\nThis is a longer solution so make sure you take the time to really understand what is happening here. To start with, before each sequence, follow along with the list above whilst you're performing each step and look at the outcome of each sequence after you've done them. It's not about just executing the moves, it's about understanding their purpose.\n\nLet's try this to see if we can insert both edge pieces correctly. You can use the <b><i>Reset</i></b> button to start the sequence over again or if you get stuck & want to start over again.\n\nRemember, it's important to <b><i>practice</i></b> this several times until you're comfortable with it, you should aim to try and solve this case without looking at the sequences if you can. Or if you're ready to move on, click <b><i>Next</i></b> to go to the next slide.",
      allowFaceMoves: true,
      setup: (cube: CubeJSWrapper) => {
        cube.reset();
    ...
    },
    {
      ...
      setup: (cube: CubeJSWrapper) => {
        cube.reset();
        // Apply setup moves: L' F2 R2 B U B2 L2 B' D' R F R D' L B L U F2 U R'
        // relative to yellow top/red front (visual)
        // Logical state is white top/green front, so remap:
        // When yellow top/red front: R→F, U→D, F→R, L→B, B→L, D→U
        // Visual: L' F2 R2 B U B2 L2 B' D' R F R D' L B L U F2 U R'
        // Logical: B' R2 F2 L D L2 B2 L' U' F R F U' B L B D R2 D F'
        cube.applyMoves([
          "B'",
          "R2",
          "F2",
